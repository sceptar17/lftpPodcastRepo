...
def test_ledger_promotes_embedded_number_when_rss_has_none(tmp_path, monkeypatch):
    ...
    episode = DiscoveredEpisode(
        episode_id="rss-known", guid="guid-known", title="Known show",
        publication_date=datetime(2012, 5, 1, tzinfo=UTC),
        source_rss_url="https://example.com/feed", audio_url="https://example.com/known.mp3",
    )
    local = LocalAudioItem(
        path=str(audio), filename=audio.name, size_mb=0.0,
        guessed_episode_number=None, match_episode_id=episode.episode_id,
        match_reason="Exact RSS enclosure filename", match_confidence=1.0,
        match_status="matched",
    )
    snapshot = InventorySnapshot(
        discovered=[episode], processed=[], missing_from_repository=[episode],
        local_audio=[local], unmatched_local_audio=[], uncertain_local_audio=[],
        feed_only_audio=[], matched_feed_count=1, local_root_configured=True,
        local_root_exists=True,
    )

    ledger = build_episode_ledger(repository, snapshot, archive)

    assert ledger.candidates[0].episode_number == 47
    assert ledger.candidates[0].episode_number_confidence == "high"
